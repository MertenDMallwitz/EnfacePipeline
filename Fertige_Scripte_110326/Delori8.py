...
def compute_segment_stats(FD, X, Y, img, img_path):
    ...
    for idx in range(len(radii_pix)):
        ...
        for i in range(n_segments):
            ...
            end_angle = (start_angle + segment_angle) % 360
            # Draw outer arc edge
            arc_outer = Arc((X,Y), 2*r_outer, 2*r_outer, theta1=start_angle, theta2=end_angle, lw=2, color='b', alpha=0.8)
            ax.add_patch(arc_outer)
            # Draw inner arc edge
            arc_inner = Arc((X,Y), 2*r_inner, 2*r_inner, theta1=start_angle, theta2=end_angle, lw=2, color='b', alpha=0.8)
            ax.add_patch(arc_inner)

            # Calculate Cartesian start/end points for connections
            for angle_deg in [start_angle, end_angle]:
                angle_rad = np.deg2rad(angle_deg)
                x_outer = X+r_outer * np.cos(angle_rad)
                y_outer = Y+r_outer * np.sin(angle_rad)
                x_inner = X+r_inner * np.cos(angle_rad)
                y_inner = Y+r_inner * np.sin(angle_rad)
                # Draw connecting lines (start and end)
                if angle_deg == start_angle:
                    ax.plot([x_inner, x_outer], [y_inner, y_outer], color='b', lw=2)
                else:
                    ax.plot([x_inner, x_outer], [y_inner, y_outer], color='b', lw=2)
...
# Example usage:
root = r"C:\Users\50126825\Desktop\octa\Drusen_algorithmus\Datenexperimente\Neuer_Ordner5"
...
map_files = find_map_files(r"C:\Users\50126825\Desktop\octa\Drusen_algorithmus\Datenexperimente\Neuer_Ordner5")
